...
public class RabbitMQConfig {
    ...
    public static final String BET_PLACED_QUEUE_NAME = "bet.placed.queue";
    public static final String BET_SETTLED_QUEUE_NAME = "bet.settled.queue";
    ...
    @Bean
    public Queue betPlacedQueue() {
        return new Queue(BET_PLACED_QUEUE_NAME);
    }

    @Bean
    public Queue betSettledQueue() {
        return new Queue(BET_SETTLED_QUEUE_NAME);
    }

    @Bean
    public Binding bindingBetPlaced(Queue betPlacedQueue, TopicExchange exchange) {
        return BindingBuilder.bind(betPlacedQueue).to(exchange).with("bet.placed");
    }

    @Bean
    public Binding bindingBetSettled(Queue betSettledQueue, TopicExchange exchange) {
        return BindingBuilder.bind(betSettledQueue).to(exchange).with("bet.settled");
    }
